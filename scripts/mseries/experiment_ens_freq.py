...
from benchmarks.parallel.time_series.fcst_single_ts_ens_freq import (
    forecast_single_ts_ens_freq,
    _get_extended_model_name,
)
...
def main(
    ...
    max_models : int, optional
        Max number of models to use in the ensemble. If the number of significant
        frequencies is less than max_models, then the number of models used will
        be less than max_models, by default "2"
    weighted : bool, optional
        Should the weights be used for the ensemble, by default "True"
        When weighted is "True", the weights are calculated as follows:
            - The weights are calculated as the inverse of the default error metric
                of the model on the training data.
    fold : Optional[int], optional
        The fold number to use for cross validation, by default "None"
        When weighted is True, `fold` is used to decide the number of cross
        validation folds to use to calculate the metric for the weights. If fold
        is set to None, then a single fold is used. When weighted is False,
        `fold` is ignored and no cross-valiation is done.
    ...
    logging.info(
        ...
        f"\nEnsemble Freq Model: '{model}', Weighted: '{weighted}', Max Models: "
        f"'{max_models}' Folds: '{fold}' Engine: '{model_engine}' "
        f"Remove_harmonics: '{remove_harmonics}' Harmonic Order Method: '{harmonic_order_method}', "
        "using ..."
        f"\n  - OS: '{OS}'"
    ...
    if weighted is True and fold is None:
        raise ValueError("`fold` must be provided when `weighted` is True")
    if max_models <= 1:
        raise ValueError("`max_models` must be greater than 1.")
    ...
    # Experiment Settings ----
    # NOTE:
    # (1) Cross validation is disabled internally based on weighted settings.
    ...
    setup_kwargs = {
        ...
        "target": "y",
        "index": "ds",
        "fold": fold or 1,  # fold cannot be None. Will ignore internally as needed
        "remove_harmonics": remove_harmonics,
        "harmonic_order_method": harmonic_order_method,
    ...
    create_model_kwargs = {
        "estimator": model,
        # "cross_validation": cross_validate,  # Set internally based on `weighted`
        "verbose": verbose,
    }
    backup_model_kwargs = {
    ...
    test_results, time_taken = execute(
        all_groups=combined,
        keys="unique_id",
        function_single_group=forecast_single_ts_ens_freq,
